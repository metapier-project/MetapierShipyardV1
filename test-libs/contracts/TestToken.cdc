...
pub contract {{token}}: FungibleToken {
    ...
    //
    pub fun createEmptyVault(): @{{token}}.Vault {
        return <- self.createTestVault(amount: 0.0)
    }

    pub fun createTestVault(amount: UFix64): @{{token}}.Vault {
    ...
    init() {
        ...
        self.TokenStoragePath = /storage/test{{token}}Vault
        self.TokenPublicReceiverPath = /public/test{{token}}Receiver
        self.TokenPublicBalancePath = /public/test{{token}}Balance
